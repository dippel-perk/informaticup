...
class ProgramArgumentsConfiguration:
    ...
    CONFIDENCE_DESCRIPTION = "The desired confidence. The algorithm will terminate, when the confidence is reached by at least one generated image. " \
                             "If no confidence is provided by the user, the algorithm will run the maximum number of steps. The confidence must be in range [0,1]."
    ...
    GTSRB_IMAGE_PATH_DESCRIPTION = "The path to the German Traffic Sign Recognition Benchmark. The images of the benchmark are used in some population generators."
    ...
    SUBSTITUTE_DESCRIPTION = "If the substitute command was added, the selected population generator will be the initial population generator for the substitute network. The tool will evolve this population to a state where the substitute network classifies the individuals to be most likely in the target class. The substitute has to be added **after the other arguments. If added, the user has two additional arguments."

    SUBSTITUTE_POPULATION_SIZE_DESCRIPTION = "The population size which should be used by the substitute network. The size has to be larger than --population-size n, because we want to use the most fit *n* individuals of the substitute network as an initial population of the genetic algorithm. If the variable is not set, we use the size 3n."
