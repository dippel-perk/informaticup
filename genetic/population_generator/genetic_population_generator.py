...
class GeneticPopulationGenerator(PopulationGenerator):
    ...
    def __iter__(self):
        ...
        genetic = GradeAverage(classifier=classifier, class_to_optimize=str(self._class_id),
                                mutation_intensity=self._mutation_intensity, average_size=self.size)
        population, _ = genetic.run(initial_population_generator=self._population_generator, steps=self._steps, grade_limit=0.99, verbose=False)
